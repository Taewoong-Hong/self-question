...
* **참여자**
  ...
  * 투표 완료 후:
    - "투표 종료 전 결과 공개" 설정 시: 투표 참여자만 중간 결과 확인 가능
    - "결과 공개" 설정 시: 모든 사람이 결과 확인 가능
    - 두 설정 모두 비활성화 시: 작성자만 결과 확인 가능
...
* 시간 설정: 시작일시, 종료일시(선택).
* 결과 공개 설정:
  * **결과 공개**: 체크 시 모든 사람이 투표 결과 확인 가능 (미체크 시 작성자만 확인 가능)
  * **투표 종료 전 결과 공개**: 체크 시 투표 참여자만 중간 결과 확인 가능 (투표하지 않은 사람은 볼 수 없음)
...
* **결과 표시 규칙**:
  - 투표하지 않은 사용자: "투표 후 결과를 확인할 수 있습니다" 메시지 표시
  - 투표한 사용자: "투표 종료 전 결과 공개" 설정 시에만 중간 결과 확인 가능
  - 모든 사용자: "결과 공개" 설정 활성화 시 결과 확인 가능
...
## 최근 업데이트 (2025-01-27)
- ✅ 투표/설문 페이지 버튼 스타일 개선 (박스 제거, 아이콘만 표시)
- ✅ 관리자가 콘텐츠 관리에서 모든 투표/설문 수정 가능
- ✅ 설문 직접입력 옵션 수정 시 문제 해결
- ✅ 중복 게시글 생성 방지 기능 추가
- ✅ 관리자 설문 수정 시 questions 배열 업데이트 오류 해결
