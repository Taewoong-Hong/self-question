...
const { Debate, DebateModel } = require('../models/DebateFirestore');
...
router.post('/create', async (req, res) => {
  try {
    ...
    const debateId = crypto.randomBytes(8).toString('hex');
    ...
    const debateData = {
      ...
      public_url: `${baseUrl}/debate/${debateId}`,
      admin_url: `${baseUrl}/debate/admin/${debateId}`
    };

    const debate = await DebateModel.create(debateData);
    debate.updateStatus();
...
router.get('/:debateId', async (req, res) => {
  try {
    const debate = await DebateModel.findById(req.params.debateId);

    if (!debate || debate.is_deleted) {
    ...
    await DebateModel.update(debate.id, { 
      stats: debate.stats,
      status: debate.status 
    });
...
router.post('/:debateId/opinion', async (req, res) => {
  try {
    ...
    debate.addOpinion(opinionData, ipHash);
...
router.get('/', async (req, res) => {
  try {
    ...
    // TODO: Firestore에서는 text search가 제한적이므로 별도 검색 솔루션 필요
    // if (search) { ... }
    ...
    // 총 개수
    const total = await DebateModel.count(filter);
...
router.put('/:debateId', authenticateAdmin, async (req, res) => {
  try {
    ...
    await DebateModel.update(debate.id, updateData);
...
router.delete('/:debateId', authenticateAdmin, async (req, res) => {
  try {
    ...
    // 소프트 삭제
    await DebateModel.softDelete(debate.id);
...
router.get('/:debateId/stats', authenticateAdmin, async (req, res) => {
  try {
    ...
    const optionStats = debate.vote_options.map(option => ({
      ...
      anonymous_votes: option.votes ? option.votes.filter(v => v.is_anonymous).length : 0,
      identified_votes: option.votes ? option.votes.filter(v => !v.is_anonymous).length : 0
    }));
